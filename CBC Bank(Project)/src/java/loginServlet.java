    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                ...
                ResultSet rs = st.executeQuery("select * from permanentCustomers where customerid='" + customerid + "'");
                if (rs.next()) {
                    String pass3 = rs.getString(13);
                    if (pass3.equals(password)) {
                        request.setAttribute("customerid", customerid);
                        String name = rs.getString(4);
                        Cookie cookie = new Cookie("custid", customerid);
                    ...
                    RequestDispatcher rd = request.getRequestDispatcher("index.jsp");
                    rd.include(request, response);
                    out.println("<h1>You are not Registered Sign Up</h1>");
                }
            } catch (Exception e) {
                out.println("<h1>" + e + "</h1>");
            }
